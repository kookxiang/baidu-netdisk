...
这是一个为偶尔使用的百度网盘 Linux 客户端准备的轻量 Docker 封装，目标是「干净、可一次性运行、无需长期在宿主机上常驻」。镜像在构建时会从网络下载百度网盘的 Linux .deb 包并解包到镜像中，运行时通过 `xpra` 提供 HTML5 访问界面，并集成 helium 浏览器用于打开分享链接。
...
docker run --rm -it \
    ...
    -v /path/to/local/config:/root/.config \
...
- `-v /path/to/local/config:/root/.config` - 挂载配置目录以保持登录状态
...
| `/root/.config` | 应用配置 | ✓ 推荐 | 保存登录状态和偏好设置 |
...
### Q: 容器占用太多磁盘空间？
A: 检查 `/root/.config` 下是否有缓存文件，可以安全删除。
