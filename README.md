...
这是一个为偶尔使用的百度网盘 Linux 客户端准备的轻量 Docker 封装，目标是「干净、可一次性运行、无需长期在宿主机上常驻」。镜像在构建时会从网络下载百度网盘的 Linux .deb 包并解包到镜像中，运行时通过 Selkies 提供 Web 访问界面，并集成 helium 浏览器用于打开分享链接。

## ✨ 功能特性

- ⚡ **按需启动** - 只在需要时启动客户端，不占用长期桌面资源
- 🌐 **Web 访问** - 通过 Selkies 暴露 Web 界面，浏览器直接访问
- 🔗 **分享链接支持** - 集成 helium 浏览器，可直接打开百度网盘分享链接
...
docker run --rm -it \
    ...
    ghcr.io/kookxiang/baidu-netdisk:latest
```

然后在浏览器打开 `http://{YOUR_IP_ADDRESS}:14500/` 即可通过 Selkies 的 Web 界面访问百度网盘应用。

**参数说明：**
- `-p 14500:14500` - 映射 Selkies Web 服务端口
- `-v /path/to/local/data:/data` - 挂载本地下载目录（可选）
- `-v /path/to/local/config:/config` - 挂载配置目录以保持登录状态
...
### Q: 如何打开百度网盘分享链接？
A: 容器内置了 helium 浏览器，您可以在浏览器中访问分享链接，并使用百度网盘客户端进行下载。

### Q: 为什么连接后无法看到应用窗口？
A: 可能是 Selkies 桌面初始化缓慢。请等待 10-15 秒，然后刷新浏览器页面。
...
- 确保定期备份重要配置（`/config` 目录）
- 容器内应用按 Selkies base 的默认用户运行，请在可信环境中使用
